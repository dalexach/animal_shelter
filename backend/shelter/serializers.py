...
class DatoVeterinarioSerializer(RegistroSaludSerializer):
    class Meta(RegistroSaludSerializer.Meta):
        ...
        fields = RegistroSaludSerializer.Meta.fields + ['procedimiento', 'resultado_examen', 'recomendaciones', 'imagenes']

class IndicadorSaludSerializer(RegistroSaludSerializer):
    class Meta(RegistroSaludSerializer.Meta):
        model = IndicadorSalud
        fields = RegistroSaludSerializer.Meta.fields + ['peso', 'vacunas', 'estado_fisico']

class ControlMedicoSerializer(RegistroSaludSerializer):
    class Meta(RegistroSaludSerializer.Meta):
        model = ControlMedico
        fields = RegistroSaludSerializer.Meta.fields + ['nombre_profesional', 'especialidad']

class SeguimientoCondicionSerializer(RegistroSaludSerializer):
    class Meta(RegistroSaludSerializer.Meta):
        model = SeguimientoCondicion
        fields = RegistroSaludSerializer.Meta.fields + ['fecha_incidente', 'diagnostico', 'tratamiento', 'evolucion']
